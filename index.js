...
module.exports = function (mongoose) {
     ...
     * @param {string} userType - Use it to declare different models (based on Mongoose Discriminators)
     * @param {function} callback
     * @return {object} the temporary user model
     */
    var generateTempUserModel = function (User, userType, callback) {
        if (!User) {
            return callback(new TypeError('Persistent user model undefined.'), null);
        }
        if (typeof (userType) === 'function') {
            callback = userType;
            userType = options.defaultModelUserType;
        }
        ...
        options.tempUserModel[userType] = mongoose.model(options.tempUserCollection + userType, tempUserSchema);

        return callback(null, mongoose.model(options.tempUserCollection + userType));
    };
     ...
     */
    var insertTempUser = function (password, tempUserData, userType, callback) {
        if (typeof (userType) === 'function') {
            callback = userType;
            userType = options.defaultModelUserType;
        }
        ...
        var newTempUser = new options.tempUserModel[userType](tempUserData);
     ...
     * @return {function} returns the callback function
     */
    var createTempUser = function (user, userType, callback) {
        if (typeof (userType) === 'function') {
            callback = userType;
            userType = options.defaultModelUserType;
        }

        if (!options.tempUserModel[userType]) {
        ...
        options.persistentUserModel.findOne(query, function (err, existingPersistentUser) {
            ...
            options.tempUserModel[userType].findOne(query, function (err, existingTempUser) {
                if (err) {
                    return callback(err, null, null);
                }
                ...
                if (existingTempUser) {
                    ...
                    if (options.hashingFunction) {
                        return options.hashingFunction(tempUserData[options.passwordFieldName], tempUserData, insertTempUser, userType, callback);
                    } else {
                        return insertTempUser(tempUserData[options.passwordFieldName], tempUserData, userType, callback);
                    }
                }
            });
        });
     ...
     * @param {function} callback
     */
    var confirmTempUser = function (url, userType, callback) {
        if (typeof (userType) === 'function') {
            callback = userType;
            userType = options.defaultModelUserType;
        }
        var TempUser = options.tempUserModel[userType],
                query = {};
     ...
     * @param {function} callback
     */
    var resendVerificationEmail = function (email, userType, callback) {
        if (typeof (userType) === 'function') {
            callback = userType;
            userType = options.defaultModelUserType;
        }
        ...
        options.tempUserModel[userType].findOne(query, function (err, tempUser) {
            if (err) {
                return callback(err, null);
            }
